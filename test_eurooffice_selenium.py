...
collabora_version = expectedResults[drv.target]["collabora"]["capabilities"][
    "productVersion"
]
minimum_version = "25.04.9.0"
try_css_selector = False
...
def checkFile(node, foldername, filename):
    ...
    while tryCount < 5:
        ...
        time.sleep(3)
    g_logger.info(
        f"File {fullPath} on {node} not found after {tryCount} tries: {exists}"
    )
    return exists
...
class TestEuroOfficeSelenium(unittest.TestCase):
    ...
    if len(drv.browsers) > 1:
        logger.warning(
            f"Please test only one browser by setting NextcloudTestBrowsers to the one you want to test: {drv.browsers}"
        )
        ...
        for collaboranode in drv.nodestotest:
            ...
            with self.subTest(mynode=collaboranode):
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error preparing nodelogin for {collaboranode}: {error}"
                    )
                    success = False
                ...
                try:
                    wait.until(
                        EC.presence_of_element_located((By.CLASS_NAME, "app-menu"))
                    )
                ...
                if not success:
                    self.logger.warning(
                        "Manually open home folder in case loading of all files takes too much time"
                    )
                ...
                try:
                    ...
                    wait.until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                '//a[@href="' + drv.indexsuffix + "/apps/files/" + '"]',
                            )
                        )
                    ).click()
                except Exception as error:
                    self.logger.warning(
                        f"Wait for {drv.indexsuffix}/apps/files/ took too long: {error}"
                    )
                    success = False
                self.assertTrue(success)
                ...
                try:
                    wait.until(
                        EC.presence_of_element_located((By.LINK_TEXT, "All files"))
                    )
                ...
                while not fileCreated:
                    ...
                    if retryCount >= g_collaboraRetryCount:
                        self.logger.error(
                            f"File {g_filename}.md has not been created after {retryCount} tries, saving screenshot"
                        )
                        screenshot = pyautogui.screenshot()
                        screenshot.save(
                            "screenshots/" + collaboranode + g_filename + ".png"
                        )
                    ...
                    try:
                        ...
                        wait.until(
                            EC.element_to_be_clickable((By.CLASS_NAME, self.addIcon))
                        ).click()
                        self.logger.info("Click on new text file")
                        time.sleep(g_clickWait)

                        button = self.driver.find_element(
                            By.XPATH, "//*[contains(text(), 'New text file')]"
                        )
                        button.click()
                        ...
                        wait.until(
                            EC.presence_of_element_located(
                                (By.XPATH, "//*[contains(@class, 'dialog__actions')]")
                            )
                        )
                        self.logger.info(
                            f"Renaming the file we just created to {g_filename}.md"
                        )
                        time.sleep(g_clickWait)
                        ...
                        self.logger.warning(
                            f"Unable to create new file: {g_filename}, saving screenshot: {error}"
                        )
                        screenshot = pyautogui.screenshot()
                        screenshot.save(
                            "screenshots/" + collaboranode + g_filename + ".png"
                    ...
                    if not fileCreated:
                        self.logger.warning(
                            f"File {g_filename}.md has not been created in try {retryCount}, refresh page and retry"
                        )
                        self.driver.refresh()
                        time.sleep(3)
                ...
                ActionChains(self.driver).send_keys(
                    f"Lorem Ipsum! {Keys.ENTER} {g_filename}"
                ).perform()
                time.sleep(
                    3
                )  # We give nextcloud a literal second to register the keystrokes before closing the document
                self.logger.info("Closing document...")
                try:
                    wait.until(
                        EC.element_to_be_clickable((By.CLASS_NAME, "header-close"))
                    ).click()
                except Exception as error:
                    self.logger.warning(f"Closing markup document failed: {error}")

                self.logger.info(
                    "Manually open home folder in case closing of document fails"
                )
                self.driver.get(drv.get_folder_url(collaboranode, ""))
    ...
    def test_eurooffice_document(self):
        delay = 30  # seconds
        global g_isLoggedIn, g_loggedInNodes, g_wait
        wait = WebDriverWait(self.driver, delay)
        g_wait = wait
            ...
            with self.subTest(mynode=collaboranode):
                ...
                for testfolder in self.testfolders:
                    with self.subTest(myFolder=testfolder):
                        self.logger.info(
                            f"TestID: {collaboranode} for folder {testfolder}"
                        )
                        success = True

                        try:
                            self.logger.info(
                                f"Wait for {drv.indexsuffix}/apps/files/ and click"
                            )
                            wait.until(
                                EC.element_to_be_clickable(
                                    (
                        ...
                        try:
                            wait.until(
                                EC.presence_of_element_located(
                                    (By.LINK_TEXT, "All files")
                                )
                            )
                            self.logger.info("All files visible!")
                        except TimeoutException:
                            self.logger.warning(
                                "Loading of all files took too much time!"
                            )
                            success = False
                        ...
                        self.logger.info(self.driver.current_url)
                        self.logger.info(
                            f"Looking for all files text in {self.version}"
                        )
                        # //*[@id="app-content-vue"]/div[1]/div/nav/ul/li/a/span/span[2] "//h4/a[contains(text(),'SAP M')]"
                        self.driver.find_element(
                            By.XPATH, "//*[contains(text(), 'All files')]"
                        )
                        self.logger.info("All files found!")

                        self.logger.info(f"Looking for {testfolder} folder")
                        folderExists = checkFolder(
                            collaboranode, testfolder, create=True
                        )
                        self.assertTrue(folderExists)
                        ...
                        while not fileCreated:
                            ...
                            if retryCount >= g_collaboraRetryCount:
                                self.logger.error(
                                    f"File {g_filename} has not been created after {retryCount} tries, saving screenshot"
                                )
                                screenshot = pyautogui.screenshot()
                                screenshot.save(
                                    "screenshots/" + collaboranode + g_filename + ".png"
                            ...
                            try:
                                ...
                                button = self.driver.find_element(
                                    By.XPATH, "//*[contains(text(), 'New document')]"
                                )
                                button.click()
                                ...
                                wait.until(
                                    EC.presence_of_element_located(
                                        (
                                            By.XPATH,
                                            "//*[contains(@class, 'dialog__actions')]",
                                        )
                                    )
                                )
                                self.logger.info(
                                    f"Renaming the file we just created to {g_filename}.docx"
                                )
                                time.sleep(g_clickWait)
                            ...
                            # Verify the file has been created
                            fileCreated = checkFile(
                                collaboranode, testfolder, g_filename + ".docx"
                            )
                            if not fileCreated:
                                self.logger.warning(
                                    f"File {g_filename}.docx has not been created in try {retryCount}, refresh page and retry"
                                )
                                self.driver.refresh()
                                time.sleep(3)
                        ...
                        try:
                            self.logger.info("Waiting for eurooffice frame")
                            wait.until(
                                EC.frame_to_be_available_and_switch_to_it(
                                    (By.CSS_SELECTOR, "iframe[id^='euroofficeFrame']")
                                )
                            )
                            self.logger.info(
                                "Euro Office loaded. Wait for the frame editor..."
                            )

                            # Euro Office has an iframe in an iframe...
                            wait.until(
                                EC.frame_to_be_available_and_switch_to_it(
                                    (By.CSS_SELECTOR, "iframe[name^='frameEditor']")
                                )
                            )
                            self.logger.info(
                                "Euro Office frame editor loaded. Wait for the editor sdk..."
                            )

                            wait.until(
                                EC.visibility_of_element_located(
                                    (By.CSS_SELECTOR, "div[id^='editor_sdk']")
                                )
                            )

                            # Wait a second until we type something..
                            time.sleep(2)
                            self.logger.info("Editor sdk found. Let's type some text")
                            ActionChains(self.driver).send_keys(
                                f"Lorem Ipsum! {Keys.ENTER} {g_filename}"
                            ).perform()
                            time.sleep(1)
                            ...
                            self.logger.error(
                                f"Error writing to the document, saving screenshot: {error}"
                            )
                            screenshot = pyautogui.screenshot()
                            screenshot.save(
                                "screenshots/" + collaboranode + g_filename + ".png"
                        ...
                        self.logger.info(
                            "Open the folder URL instead of closing the document"
                        )
                        self.driver.get(drv.get_folder_url(collaboranode, ""))
                        self.assertTrue(success)
    ...
    # spreadsheet
    def test_eurooffice_spreadsheet(self):
        delay = 30  # seconds
        global g_isLoggedIn, g_loggedInNodes, g_wait
        wait = WebDriverWait(self.driver, delay)
        g_wait = wait
            ...
            with self.subTest(mynode=collaboranode):
                ...
                while not fileCreated:
                    ...
                    try:
                        ...
                        button = self.driver.find_element(
                            By.XPATH, "//*[contains(text(), 'New spreadsheet')]"
                        )
                        button.click()
                        ...
                        self.logger.info(
                            f"Renaming the file we just created to {g_filename}.xlsx"
                        )
                        time.sleep(g_clickWait)
                        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys(
                            "a"
                    ...
                    if not fileCreated:
                        self.logger.warning(
                            f"File {g_filename}.xlsx has not been created in try {retryCount}, refresh page and retry"
                        )
                        self.driver.refresh()
                        time.sleep(3)
                ...
                try:
                    self.logger.info("Waiting for eurooffice")
                    wait.until(
                        EC.frame_to_be_available_and_switch_to_it(
                            (By.CSS_SELECTOR, "iframe[id^='euroofficeFrame']")
                        )
                    )
                    self.logger.info("Euro Office loaded. Wait for the frame editor...")
                    ...
                    ActionChains(self.driver).send_keys(
                        f"{g_filename}{Keys.ENTER}{Keys.SPACE}1{Keys.ENTER}{Keys.SPACE}2{Keys.ENTER}{Keys.SPACE}3{Keys.ENTER}{Keys.SPACE}4{Keys.ENTER}{Keys.SPACE}"
                    ).perform()
                    time.sleep(1)
                except Exception as error:
                    self.logger.error(
    ...
    # presentation
    def test_eurooffice_presentation(self):
        delay = 30  # seconds
        global g_isLoggedIn, g_loggedInNodes, g_wait
        wait = WebDriverWait(self.driver, delay)
        g_wait = wait
        ...
        for collaboranode in drv.nodestotest:
            ...
            with self.subTest(mynode=collaboranode):
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Loading of all files took too much time: {error}"
                    )
                    success = False
                ...
                while not fileCreated:
                    ...
                    try:
                        ...
                        button = self.driver.find_element(
                            By.XPATH, "//*[contains(text(), 'New presentation')]"
                        )
                        button.click()
                        ...
                        self.logger.info(
                            f"Renaming the file we just created to {g_filename}.pptx"
                        )
                        time.sleep(g_clickWait)
                        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys(
                            "a"
                    ...
                    if not fileCreated:
                        self.logger.warning(
                            f"File {g_filename}.pptx has not been created in try {retryCount}, refresh page and retry"
                        )
                        self.driver.refresh()
                        time.sleep(3)
