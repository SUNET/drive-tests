        ...
        for fullnode in drv.fullnodes:
            with self.subTest(mynode=fullnode):
                for fe in range(1,4):
                    ...
                    try:
                        for i in range(0,numFiles,maxUploads):
                            ...
                            for file in files[x:x+maxUploads]:
                                try:
                                    g_testThreadsRunning += 1
                                    client.upload_async(remote_path=f'largefiles/{file}',local_path=f'{targetDirectory}/{file}', callback=decreaseUploadCount)
                    ...
                    davElements = client.list('largefiles')
                    ...
                    for i in range(0,len(davElements),maxDeletes):
                        ...
                        for element in davElements[x:x+maxDeletes]:
                            t = threading.Thread(target=client.clean, args=['largefiles/' + element])
                            t.start()
