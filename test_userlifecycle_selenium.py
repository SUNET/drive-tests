                ...
                for fullnode in drv.nodestotest:
                    with self.subTest(mynode=fullnode):
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"Error posting to create cli user {error}"
                            )
                            return
                        try:
                            j = json.loads(r.text)
                            ...
                            if j["ocs"]["meta"]["statuscode"] == 102:
                                ...
                                r = session.post(
                                    addUserUrl, headers=ocsheaders, data=data
                                )
                                j = json.loads(r.text)
                                self.logger.info(
                                    json.dumps(j, indent=4, sort_keys=True)
                                ...
                                if j["ocs"]["meta"]["statuscode"] != 100:
                                    self.logger.info(
                                        f"Retry to delete cli user after {lifecycleuser} after error {j['ocs']['meta']['statuscode']}"
                                    )
                                    r = session.delete(userurl, headers=ocsheaders)
                                    j = json.loads(r.text)
                                    self.logger.info(
                                ...
                                self.logger.info(
                                    f"Retry to create cli user {lifecycleuser} after error {j['ocs']['meta']['statuscode']}"
                                )
                                r = session.post(
                                    addUserUrl, headers=ocsheaders, data=data
                                )
                            ...
                            self.logger.info(
                                f"No JSON reply received from {fullnode}: {error}"
                            )
                            self.logger.info(r.text)
                            return

                        self.logger.info(
                            f"TestID: Testing node {fullnode} with browser {browser}"
                        )
                        ...
                        newOtpSecret = sel.nodelogin(
                            sel.UserType.BASIC,
                            username=lifecycleuser,
                            password=lifecyclepwd,
                            mfaUser=mfaUser,
                            addOtp=mfaUser,
                            totpsecret=newOtpSecret,
                        )
                        wait = WebDriverWait(driver, delay)

                        lifecycleapppwd = sel.create_app_password()
                        self.logger.info(
                            f"Got app password for {fullnode}: {lifecycleapppwd} and secret {newOtpSecret}"
                        )
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"Webdav error making shared folder {error}"
                            )

                        files = driver.find_element(
                            By.XPATH,
                        ...
                        try:
                            wait.until(
                                EC.presence_of_element_located(
                                    (By.CLASS_NAME, sharedClass)
                                )
                            )
                            sharefolder = driver.find_element(
                                by=By.CLASS_NAME, value=sharedClass
                            )
                            sharefolder.click()
                        ...
                        try:
                            wait.until(
                                EC.presence_of_element_located(
                                    (By.CLASS_NAME, "sharing-entry__title")
                                )
                            )
                        ...
                        try:
                            wait.until(
                                EC.element_to_be_clickable((By.ID, "user-menu"))
                            ).click()
                            self.logger.info("user-menu clicked")
                            wait.until(
                                EC.element_to_be_clickable(
                                    (By.PARTIAL_LINK_TEXT, "Settings")
                                )
                            ).click()
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"Problem accessing security settings: {e}"
                            )
                        ...
                        wait.until(
                            EC.element_to_be_clickable((By.ID, "old-pass"))
                        ).send_keys(lifecyclepwd)
                        wait.until(
                            EC.element_to_be_clickable((By.ID, "new-pass"))
                        ).send_keys(lifecyclenewpwd)
                        time.sleep(1)
                        wait.until(
                            EC.element_to_be_clickable((By.ID, "new-pass"))
